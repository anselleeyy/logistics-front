layui.use(['layer', 'form', 'element', 'jquery', 'table'], function() {
    ...
    let tableArray = [ '#receiveTable', '#receiveTable1' ];
    let tableUrl = [ '/goodsBill/findInform/', '/goodsBill/findOldInform/' ];
    ...
    function refreshTable(id) {
        table.render({
            elem: tableArray[id],
            height: 'full-170',
            url: nginx_url + tableUrl[id] + '到货回告', //数据接口
            limit: 10,
            limits: [ 10 ],
            request: {
    ...
    table.on('tool(billTool)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'confirm') { //填写司机回执并且发出到货通知
            layer.open({
                type: 2,
                title: data.goodsBillCode + ' - 货物到货回告填写',
                content: [ '../writeInform.html?id=' + data.goodsBillCode + '&type=到货回告' ],
                area: [ '85%', '85%' ],
                shadeClose: true,
                move: false,
                end: function() {
                    table.reload('receiveTable', {
                        url: nginx_url + tableUrl[0] + '到货回告'
                    })
                }
            });
        } else {
            layer.open({
                type: 2,
                title: data.goodsBillCode + ' - 货物到货回告详情',
                content: [ '../informDetail.html?id=' + data.goodsBillCode + '&type=到货回告' ],
                area: [ '65%', '65%' ],
                shadeClose: true,
                move: false,
                end: function() {
                    table.reload('receiveTable1', {
                        url: nginx_url + tableUrl[1] + '到货回告'
                    })
                }
            });
        }
